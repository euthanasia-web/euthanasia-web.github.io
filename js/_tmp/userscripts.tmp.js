...
(function () {
  var body = document.querySelector("body"); // if (body.offsetWidth >= 1260) {
  ...
  var progressScene = new ScrollMagic.Scene({
    ...
    duration: progressContainer.offsetHeight
  ...
  }).on("enter", function (e) {// $("#roadmap-step-1").text(e.type == "enter" ? "inside" : "outside");
  }).on("start end", function (e) {// $("#lastHit").text(e.type == "start" ? "top" : "bottom");
  ...
  var initStep = function initStep(number) {
    new ScrollMagic.Scene({
      triggerElement: "#roadmap-step-" + number
    }) // .addIndicators() // add indicators (requires plugin)
    .addTo(controller).on("enter", function (e) {
      $("#roadmap-step-" + number).addClass("roadmap-step--active"); // $("#roadmap-step-" + number + " .js-step-preview").addClass("js-step-preview-active");
    }).reverse(false);
  };

  var initCheckpoint = function initCheckpoint(step, number) {
  ...
  steps.forEach(function (step, i) {
    initCheckpoint(step, i + 1); // initStep(i + 1);
  });
})();

(function () {
  var lang = 'eng';
  var minPrice = lang === 'eng' ? 16 : 365;
  ...
  var initCode = function initCode() {
    ...
    if (code === 'партакиада') {
      priceEl.innerHTML = minPrice;
    } else if (code.toString()[0] === '!') {
      var customPrice = code.slice(1);
